...
function RouteComponent() {
  ...
  const { registerUserMutation } = useUpdateUserAuth();
  const passwordStrength = getPasswordStrength(passwordValue);

  const formFields = [
    { name: "name" as const, delay: 0.3 },
    { name: "email" as const, delay: 0.4 },
    { name: "password" as const, delay: 0.5 },
    { name: "confirmPassword" as const, delay: 0.6 },
  ] as const;
    ...
    try {
      ...
      await registerUserMutation.mutateAsync(registerData, {
        ...
        onError: (error) => {
          ...
          setRegisterError("Произошла ошибка при регистрации. Пожалуйста, попробуйте снова.");
        },
      });
    } catch (error) {
      console.error("Ошибка при регистрации:", error);
        ...
        <div className="relative container mx-auto flex min-h-[calc(100vh-8rem)] max-w-2xl flex-col items-center justify-center px-4">
          ...
          >
            <div className="mb-8 text-center">
              ...
              >
                Создайте аккаунт для доступа к программам лояльности
              </m.p>
            </div>
            ...
            >
              <div className="p-4 sm:p-6">
                {registerError && (
                  <div className="mb-4 rounded-md bg-red-500/10 p-3 text-sm text-red-500">{registerError}</div>
                )}
                  ...
                  {formFields.map((field) => (
                    ...
                    >
                      {field.name === "password" ? (
                        ...
                        <TInput
                          label={field.name === "name" ? "Имя" : "Почта"}
            ...
            >
              ...
              <div className="mt-6 flex flex-col gap-4">
                <m.div whileHover={{ scale: 1.005 }} whileTap={{ scale: 0.98 }}>
                  <Link to="/user/login" className="flex items-center justify-center gap-2">
